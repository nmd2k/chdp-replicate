#!/usr/bin/env python3
"""
Main training script for CHDP (Codebook-based Hierarchical Diffusion Policy)
Reproduces results from arXiv:2601.05675
"""
...
class CHDPTrainer:
    """Trainer for CHDP agent."""
    
    def __init__(self, config: dict, env_name: str, seed: int, log_dir: str):
        self.config = config
        ...
        self.agent = self._create_agent(state_dim, action_dim, param_dim)
    ...
    def _create_agent(self, state_dim: int, action_dim: int, param_dim: int):
        ...
        return CHDPAgent(
            ...
            codebook_size=self.config["architecture"]["codebook_size"],
            ...
            sequential_update=self.config["update"]["sequential"],
...
def run_experiment(env_name: str, seed: int, config: dict, results_dir: str) -> dict:
    """Run single experiment."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = os.path.join(results_dir, f"{env_name}_seed{seed}_{timestamp}")
    
    trainer = CHDPTrainer(config, env_name, seed, log_dir)
...
def main():
    parser = argparse.ArgumentParser(description="Train CHDP agent")
    ...
    parser.add_argument("--results-dir", type=str, default="results/chdp",
    ...
    for env_config in environments:
        ...
        for seed in seeds:
            print(f"\nRunning seed {seed}...")
            result = run_experiment(env_name, seed, config, args.results_dir)
            env_results.append(result["final_success_rate"])
        
        mean_score = np.mean(env_results)
        std_score = np.std(env_results)
        ...
        print(f"\n{env_name}: {mean_score:.1f} ± {std_score:.1f}")
        
        if env_name in config.get("targets", {}):
            target = config["targets"][env_name]
            print(f"Target: {target['mean']:.1f} ± {target['std']:.1f}")
